# Plan: one resource planning in a new schema `plan`
...
Built next to what runs, in a new schema `plan`, so every step is additive until the
switch, and the switch itself is one script with a matching rollback script.

## 2. the new model

```
plan.plan                       one row per line type (+ tenant set): the board scope
  plan_id            bigint identity pk
  line_type          text not null
  tenant_ids         integer[] not null
  unique (line_type, tenant_ids)

plan.lane                       one strip of time: one machine on one day for one step
  lane_id            bigint identity pk
  plan_id            bigint not null references plan.plan
...
plan.lane_item                  the block of work on a lane
  lane_item_id                bigint identity pk
  lane_id                     bigint not null references plan.lane
  ...
  is_pinned                   boolean not null default false -- see §6.4
...
plan.lane_item_dependency       many-to-many, from = predecessor
  from_lane_item_id  bigint not null references plan.lane_item on delete cascade
  to_lane_item_id    bigint not null references plan.lane_item on delete cascade
  primary key (from_lane_item_id, to_lane_item_id), index (to_lane_item_id)

plan.lane_item_event            append-only status moves (plan -> released -> nested)
  lane_item_event_id bigint identity pk
  lane_item_id       bigint not null references plan.lane_item on delete cascade
...
| `day_offset`, `lag_formula` | read columns, not stored (§3.3, §6.2) |
...
- `summary` is written, not computed at read time: `plan.refresh_lane_item_data` recomputes
...
| part | writer | when |
|---|---|---|
| `material_id`, `imposition_group_id`, `nest_moment_code`, `fixed_group`, `no_split`, `i18n` | `plan.generate_day` | stamping the day from `mock.material_print_schedule` |
| `production_orderlines`, `summary` | `plan.refresh_lane_item_data` | daily refresh, for items not yet released; and after every `batches` change |
| `batches` | `plan.place_nests` (from `legacy.crud_nest`) | a nest is nested: it lands on the impose item of its material, moment and day (rule of 9 Sep: released item, first moment at or after `nested_at`) |
| step items + `lane_item_dependency` | `plan.place_nests` | same moment: per step and resource_path in `legacy.nest.manifest_json`, one item on the lane `(lane_date, step, resource_path)` of the same plan, created when missing, and an edge from the impose item. The step item has `data_json` null |
| `sort_order`, `start_offset_in_seconds`, `is_pinned`, lane move, copy, delete | `plan.crud_lane_item` | the board (`docs/contracts/drag-and-drop.md`) |
| `lane_item_event` | `plan.crud_lane_item_event` | release on the board, `nested` from `place_nests` |

### 3.3 inherited data_json

A step item carries no `data_json`. The read resolves it: `plan.get_lane_item_data(p_lane_item_id)`
...
deviates, via `plan.crud_lane_item`; from then on that item is the source for its successors.
...
|---|---|---|
| `plan.get_lanes(p_until, p_line_type, p_tenant_ids, p_steps, p_view_code)` | `action.get_plan_lanes_imposition_group`, `action.get_plan_lanes_resource` | one row per lane: `lane_id`, `lane_date`, `day_offset`, `step`, `resource_path`, `resource_uid`, `resource_name`, `tenant_id`, `sort_order`, `formula` (from `production.resource_setting`) |
| `plan.get_lane_items(p_until, p_line_type, p_tenant_ids, p_steps, p_types, p_view_code)` | `action.get_resource_plan`, `mock.get_impose_plan` | one row per lane item and type: the columns of §2 plus `day_offset`, `lag_formula` (§6.2), `type_json`, `data_json` (own or inherited), `class_names`; `progress` and `actual` rows derived as `get_resource_plan` does today |
| `plan.get_lane_item_data(p_lane_item_id)` | `get_lane_item_impositions` (never live) | the effective `data_json` |
| `plan.get_inflow(...)` | `mock.get_impose_plan_inflow` | 79: pass-through of `mapping.get_production_orderline_manifest` plus `lane_item_id` of the first unreleased item |

Writes, all set-based over `jsonb_array_elements(p_param_json) AS el`:

| function | replaces |
|---|---|
| `plan.generate_day(p_date, p_line_type)` | `mock.generate_plan`, `mock.generate_production_plan` |
| `plan.refresh_lane_item_data(p_lane_item_ids)` | the aggregate fold in `get_impose_plan` / `get_lane_item_work` |
| `plan.place_nests(p_nest_ids)` | the `batch_lane_item` block in `legacy.crud_nest`, `action.sync_pv2_batch_items` |
| `plan.crud_lane_item(p_param_json)` | `action.crud_lane_item` |
| `plan.crud_lane_item_event(p_param_json)` | `action.crud_lane_item_event` |
...
Every step ships as `sql/plan/<nn>_<name>.sql` (up) and `sql/plan/<nn>_<name>_down.sql`
(rollback). Nothing in a step touches `action.*` until step 7. I deliver, you run, I wait.

| step | what | rollback | done when |
|---|---|---|---|
| 0 | decisions of §6 taken; `legacy.nest.manifest_json` exists and carries `step` and `resource_paths` per scope (§6.1); `action.formula` gets its lag rows (§6.2) | n/a | you confirm |
| 1 | schema `plan` with the five tables of §2, owner `xfw3`, comments in English; lookups stay where they are | `drop schema plan cascade` | tables exist, empty |
...
| 3 | `plan.generate_day` + `plan.refresh_lane_item_data`; backfill the same window as `site.refresh_derived_data` (today + 14 days, every line type) in a DO block with RAISE NOTICE; `refresh_derived_data` calls `plan.generate_day` **next to** `mock.generate_plan` | drop functions, `truncate plan.lane_item, plan.lane, plan.plan`, remove the call | per day and line type: same materials × nest moment codes as `action.lane_item` with source `material-plan` |
| 4 | `plan.place_nests`; `legacy.crud_nest` calls it **next to** its `batch_lane_item` block; backfill nests of the window; `plan.crud_lane_item`, `plan.crud_lane_item_event`; release events copied from `action.lane_item_event` for the window | remove the call, drop functions, `truncate plan.lane_item_dependency, plan.lane_item_event` | every `batch_lane_item.nest_ids` of the window is in exactly one `data_json.batches[].nest_ids`; step items and edges exist per manifest step |
| 5 | data side: new data_groups next to the old ones, same layouts, `src` on the `plan` data_tables: `plan_lane_items` (76 + 81 as one board, steps per page as `params` on the section), `plan_lane_items_filter` (82), `plan_inflow` (79), `plan_print_schedule` (75 on `plan.get_lanes`); pages `nest-plan`, `production-plan` next to the current pages; frontend handoff §7 | delete the new data_groups and pages | both new pages load with the same labels and items as 76/81 |
| 6 | parallel run, at least a full week of nesting. Read-only compare script `sql/plan/check_parallel.sql`: items per day/step/resource, nests per item, `summary.sqm` against `get_lane_item_work.sqm`, released items per day | n/a | differences explained or fixed |
| 7 | switch, one script: nav and pages point at the new data_groups; `crud_nest` drops the `batch_lane_item` block; `refresh_derived_data` drops `mock.generate_plan`; `log.upsert_state_shift_agg` reads `planned_output_sqm` from `plan.lane_item` (§6.7); old data_groups archived to `archive/data_group/` | the same script mirrored: nav back, blocks back, data_groups restored; the `plan` schema keeps running so nothing is lost | boards in use for a week without a rollback |
...
1. **`legacy.nest.manifest_json`.** It is not in the repo DDL and I could not read the live
   table. The plan needs per scope: `item_code_paths`, `step`, `resource_paths` (the candidate
   machines), `production_impact_per_unit`, `config`. Proposal: `legacy.create_imposition_unit_manifest`
   ...
   `production.resource_setting` of the group and step). Confirm, or give the shape you have.
2. **`lag_formula`.** You wrote `p_view_type`; the existing parameter is `p_view_code`
   ...
   `catalog.get_formula`).
3. **Plan grain.** One `plan.plan` per `(line_type, tenant_ids)`, no date, no steps. A lane
...
4. **`is_pinned`.** Not in your list. The drop contract writes it as a column next to
   `sort_order` and `start_offset_in_seconds`; proposal: keep it a column. Alternative: `data_json.is_pinned`
   with dot-notation in `is_pinned_field`.
5. **`day_offset`.** Proposal: a read column (`lane_date` minus the view day), not stored;
   stored it would need a nightly update of every item.
...
7. **Readers of `action.object`** that are not planning boards and need a new home before
   ...
   `legacy.get_nest_status_by_bucket`. Proposal: the log side reads `plan.lane_item` with
...
8. **Copy semantics** of Ctrl-drop and split/merge on the board: a copy takes `data_json`
   ...
   `plan.crud_lane_item_dependency` for the board.
...
- `drop`: `order_field sort_order`, `is_pinned_field is_pinned`, `no_split_field data_json.no_split`,
  `within_fields ⊆ group_by` as before; mutation goes to `plan.crud_lane_item`
