-- ============================================================
-- get_impose_plan (76) serves start_at (6 sep): the absolute start of the row,
-- the plan date's local midnight plus start_offset_in_seconds, null for a
-- filler without offset. The board's formulas compare it with
-- current_offset_in_seconds; data_group 76 names it in start_at_field and
-- points set_date_field at nest_date (there is no column date). Run the
-- partial for 76 afterwards.
-- ============================================================
...
-- check: every material row with an offset has a start_at on the plan date
SELECT count(*) AS rows, count(start_at) AS with_start_at, min(start_at) AS first, max(start_at) AS last
