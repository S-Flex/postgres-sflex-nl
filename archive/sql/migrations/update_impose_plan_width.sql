-- ============================================================
-- get_impose_plan (76): the width of a material row — duration_in_seconds
-- and the formula variable production_impact_in_seconds — counts the work of
-- the 30-hour delivery class only (7 sep). The other classes still ride along
-- in the numbers of the row (sqm, orderlines, statuses); they no longer add to
-- its time. The class is the variable v_width_delivery_hours in the function.
-- Applies to both paths of the row: the open work of a material without
-- nests and the work of a planned nest set. Return type unchanged.
-- ============================================================
...
begin
    ...
    select r.material_id, r.material_name, r.production_line_id,
           ...
           -- standard production impact of its orderlines of the width class
           -- (30 hours, from the manifests), never shorter than the floor. The machine formula
...
-- check: the rows of monday 7 sep, their width against the sum of all classes
SELECT r.material_id, r.fixed_group, r.duration_in_seconds, r.orderline_count, r.sqm
FROM mock.get_impose_plan(p_until => '2026-09-07 06:00+02', p_line_type => 'sheet') r
WHERE r.material_id IS NOT NULL ORDER BY r.duration_in_seconds DESC LIMIT 10;
