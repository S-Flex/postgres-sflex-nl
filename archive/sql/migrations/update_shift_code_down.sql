-- Rollback of sql/update_shift_code.sql: the code leaves shift_json, the
-- upsert goes back to the version before (no shift_code, one output area), the
-- columns go back, the aggregate is rebuilt from 7 Sep 2026.
BEGIN;

UPDATE action.dates d
SET shift_json = (SELECT jsonb_agg(sh.value - 'code' ORDER BY sh.ordinality)
                  FROM jsonb_array_elements(d.shift_json) WITH ORDINALITY AS sh(value, ordinality))
WHERE d.shift_json IS NOT NULL
  AND d.shift_json::text LIKE '%"code"%';

-- ============ sql/log/upsert_state_shift_agg.sql, the version before ============
...
ALTER TABLE log.state_shift_agg DROP COLUMN IF EXISTS shift_code;
ALTER TABLE log.state_shift_agg DROP COLUMN IF EXISTS actual_net_output_sqm;
ALTER TABLE log.state_shift_agg RENAME COLUMN actual_gross_output_sqm TO actual_output_sqm;
