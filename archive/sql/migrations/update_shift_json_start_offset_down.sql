-- Rollback of sql/update_shift_json_start_offset.sql: the key back to
-- start_offset_in_seconds, break_times removed, the totals read the version of 8 Sep 2026, the aggregator the plan_calibrated version on the old key.
BEGIN;

UPDATE action.dates d
SET shift_json = coalesce(
    (SELECT jsonb_agg(
                (t.e - 'start_offset' - 'break_times')
                || jsonb_build_object('start_offset_in_seconds', coalesce(t.e -> 'start_offset', t.e -> 'start_offset_in_seconds'))
                ORDER BY t.o)
...
-- ============ sql/log/upsert_state_shift_agg.sql (plan_calibrated version, old key) ============
...
-- ============ sql/log/get_resource_state_shift_totals.sql (8 Sep 2026) ============
