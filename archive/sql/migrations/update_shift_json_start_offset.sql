-- action.dates.shift_json (13 Sep 2026): the key start_offset_in_seconds
-- becomes start_offset (seconds are the contract, no unit in a key) and every
-- shift gets break_times: 3600 (the break seconds inside the shift).
--   before: {"tenants": [1, 2], "start_time": "06:00", "shift_duration": 32400, "start_offset_in_seconds": 21600}
--   after:  {"tenants": [1, 2], "start_time": "06:00", "shift_duration": 32400, "start_offset": 21600, "break_times": 3600}
-- 1. the data: every element of every row, order kept; a row already in the
--    new shape is left as it is (rerunnable)
-- 2. the two readers of the key: log.upsert_state_shift_agg (the version with
--    the plan_calibrated row, update_plan_calibrated.sql) and
--    log.get_resource_state_shift_totals. Neither reads break_times yet.
-- 3. rebuild of yesterday and today: the aggregator live since
--    update_plan_calibrated.sql already read start_offset, so 12 and 13 Sep
--    have no rows until this runs.
-- Rollback: sql/update_shift_json_start_offset_down.sql.
BEGIN;

-- ============ action.dates ============
UPDATE action.dates d
SET shift_json = coalesce(
    (SELECT jsonb_agg(
                (t.e - 'start_offset_in_seconds')
                || jsonb_build_object(
                       'start_offset', coalesce(t.e -> 'start_offset_in_seconds', t.e -> 'start_offset'),
                       'break_times',       coalesce(t.e -> 'break_times', '3600'::jsonb))
                ORDER BY t.o)
     FROM jsonb_array_elements(d.shift_json) WITH ORDINALITY AS t(e, o)),
    '[]'::jsonb)
WHERE d.shift_json IS NOT NULL
  AND jsonb_typeof(d.shift_json) = 'array';
...
declare
  ...
  -- evaluate_many_nas runs these lines over them. The buckets are
  -- producing, available (idle, starved, blocked: the machine could run
  -- and does not, the loss the OEE measures, inside production_in_seconds
  -- without being summed), unavailable (breakdown, maintenance,
  -- interruption, missingdata, installation; offline counts here too) and
  -- planned. Every time is in seconds, the frontend formats them as hh:mm
  v_formula_json jsonb := jsonb_build_array(
      ...
      -- its own area (unavailable selected in the filter), so it
      ...
      -- out of available, a selected unavailable state out of unavailable
      ...
      'unavailable_percentage = total_shift_in_seconds > 0 ? unavailable_in_seconds / total_shift_in_seconds * 100 : 0',
...
begin
  ...
  -- start_offset after midnight and lasts shift_duration seconds,
  ...
  shift_def as (
    select d.date as shift_date,
           ...
           (d.date::timestamp + make_interval(secs => (sh.value ->> 'start_offset')::integer))
             at time zone 'Europe/Amsterdam' as shift_start,
           (d.date::timestamp + make_interval(secs => (sh.value ->> 'start_offset')::integer
                                                    + (sh.value ->> 'shift_duration')::integer))
             at time zone 'Europe/Amsterdam' as shift_end,
  ...
  -- 'producing' draws setup too and 'unavailable' draws missingdata
  bucket_sums as (
    select b.shift_date, b.shift_index, b.resource_uid, b.step, b.line,
           ...
           sum(b.seconds) filter (where sm.counts_as in ('unavailable', 'offline')) as unavailable_seconds,
           sum(b.seconds) filter (where sm.counts_as = 'planned')   as planned_seconds,
           -- the losses (the available bucket) drawn as their own
           -- area: they move out of the available band
           sum(b.seconds) filter (where sm.counts_as = 'available'
           ...
           sum(b.seconds) filter (where sm.counts_as in ('unavailable', 'offline')
                                    and (p_states is null or b.state = any(p_states)
  ...
  oee as (
    select t.shift_date, t.shift_index, t.resource_uid, t.resource_name,
           ...
           greatest(coalesce(bs.unavailable_seconds, 0)
           ...
           greatest(t.total_seconds
                    - coalesce(bs.unavailable_seconds, 0)
    ...
    cross join lateral (
      ...
      select jsonb_build_object(
                 ...
                 'unavailable_in_seconds',       round(coalesce(bs.unavailable_seconds, 0), 0),
  ...
  final_rows as (
    ...
    -- one synthetic 'unavailable' row per group: the unavailable time
    -- not already shown as its own area, at the top of the stack.
...
-- ============ check ============
-- expected: one key set: break_times,shift_duration,start_offset,start_time,tenants; 0 old keys
SELECT string_agg(DISTINCT ks, ' | ') AS key_sets,
       count(*) FILTER (WHERE e ? 'start_offset_in_seconds') AS old_keys,
       count(*) AS shifts
FROM action.dates d
CROSS JOIN LATERAL jsonb_array_elements(d.shift_json) AS e
CROSS JOIN LATERAL (SELECT string_agg(k, ',' ORDER BY k) FROM jsonb_object_keys(e) k) AS x(ks);

-- expected: rows for today, as before the change
SELECT count(*) AS totals
FROM log.get_resource_state_shift_totals(p_resource_uids := NULL, p_until := now());
