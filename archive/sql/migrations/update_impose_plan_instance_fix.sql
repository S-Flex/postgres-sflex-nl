-- Fix on step 3 (sql/update_batch_lane_item_readers.sql): the base step of
-- mock.get_impose_plan listed its columns by name and missed the two new
-- ones, so the final select failed at run time (column r.instance does not
-- exist). Only this function; the rest of step 3 is in.
BEGIN;
