-- Two columns on log.state_shift_agg (decided 9 sep 2026): the nest sheet
-- area planned and produced per resource and shift, from the nest size
-- (legacy.nest width x height in cm), never from the orderline sqm.
--   planned_output_sqm  on the planned row: the pv2 items (action.object,
--                       printers and cutters) starting in the shift,
--                       batched_amounts amount x nest size
--   actual_output_sqm   on the producing row: the log.data jobs (print and
--                       cut) starting in the shift, amount x nest size
-- Every other row carries null. A cutter without a plan has no planned row
-- and only the actual on its producing row; a machine that produced without
-- a logged running state gets a producing row with 0 seconds and its area.
-- The planned state now covers cutters as well (it was printers only).
-- The upsert is rebuilt, then the last 42 days (what the boards read) are
-- rebuilt day by day.
BEGIN;

ALTER TABLE log.state_shift_agg
    ADD COLUMN IF NOT EXISTS planned_output_sqm numeric,
    ADD COLUMN IF NOT EXISTS actual_output_sqm numeric;

COMMENT ON COLUMN log.state_shift_agg.planned_output_sqm IS 'Nest sheet area (m2) planned on the resource in this shift: pv2 items starting in it, batched_amounts x nest width x height. On the planned row only.';
COMMENT ON COLUMN log.state_shift_agg.actual_output_sqm IS 'Nest sheet area (m2) produced in this shift: log.data jobs starting in it, amount x nest width x height. On the producing row only.';
...
-- rebuild the last 42 days; one notice per day
DO $$
DECLARE
    v_day  date;
    v_rows integer;
BEGIN
    FOR v_day IN SELECT d::date FROM generate_series(current_date - 42, current_date, interval '1 day') d LOOP
        v_rows := log.upsert_state_shift_agg(v_day);
        RAISE NOTICE 'state_shift_agg %: % rows', v_day, v_rows;
    END LOOP;
END $$;

-- check: output per day; expected planned on planned rows, actual on producing rows, nothing elsewhere
SELECT shift_date,
       round(sum(planned_output_sqm) FILTER (WHERE state = 'planned'))   AS planned_sqm,
       round(sum(actual_output_sqm)  FILTER (WHERE state = 'producing')) AS actual_sqm,
       count(*) FILTER (WHERE state NOT IN ('planned', 'producing')
                          AND (planned_output_sqm IS NOT NULL OR actual_output_sqm IS NOT NULL)) AS misplaced
FROM log.state_shift_agg
WHERE shift_date >= current_date - 7
