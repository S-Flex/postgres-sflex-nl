-- Two rules for the items of the lanes read (action.get_plan_lanes_imposition_group):
--
-- 1. A day before day 0 is history: every item of it has its time (its own,
--    else the moment of its delivery class) and is pinned. Until now 19 of the
--    23 items of yesterday (tenant 1, 9 sep) were fillers with a null offset,
--    so the client chained them into today's chain by sort_order.
-- 2. An item with nests counts as pinned: it keeps its own time, else the clock
--    time of its first nest on its day. The items without nests stay fillers
--    and flow with the clock, so a nested row stays where it was nested and the
--    free rows swap past it.
--
-- mock.get_impose_plan reads this function and needs no change.
BEGIN;
