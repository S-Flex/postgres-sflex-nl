-- The inflow queue (79) shows the fill of an imposition of the material: 100
-- minus the waste factor of its nest group in catalog.imposition_group, the
-- widest format at its best factor. mapping.get_production_orderline_manifest
-- returns it as fill_percentage on every row; the return type changes, so the
-- function is dropped and created.
BEGIN;
