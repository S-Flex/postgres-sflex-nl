-- The states filter of the OEE boards and the legend of the OEE charts read
-- the lookup instead of a copy in their config. Board 64 in the database still
-- offered the old list (running, idle, ...), the repo list of 6 September was
-- never applied; both are gone now, the list is the lookup's.
--
-- 1. log.get_resource_states: the nodes of lookup_resource_state that carry
--    counts_as, in lookup order, with i18n, class_name, fill and color.
-- 2. site.data_table get_resource_states, so a data group can name it as src.
-- 3. The data groups (sql/update_data_group_partial.sql, run after this):
--    64 states filter input_data.src = get_resource_states (title_field i18n,
--    value_field code); 29 legend from the same src; 62 set_title_field
--    state_json.i18n for its legend.
BEGIN;
...
SELECT 'get_resource_states', 'log.get_resource_states', '', 'get_resource_states',
       jsonb_build_object('primary_keys', jsonb_build_array('code')), false
WHERE NOT EXISTS (SELECT 1 FROM site.data_table t WHERE t.data_table = 'get_resource_states');
