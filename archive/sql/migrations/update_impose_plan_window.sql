-- ============================================================
-- get_impose_plan (76): p_look_back_days and p_look_ahead_days default to 1
-- (were 0): the open work behind a material row spans the day before and the
-- day after the plan date as well. The impose_plan_filter (80) carries both
-- as hidden selects (look_back_days, look_ahead_days, default 1), board 76
-- passes them on. Signature default changes, so drop + create.
-- ============================================================
...
-- check: the window widens the work behind the rows — the same call with 0, 1 and 2 days each side
SELECT w.days, count(*) FILTER (WHERE r.material_id IS NOT NULL) AS material_rows,
       sum(r.orderline_count) AS orderlines, round(sum(r.sqm)) AS sqm, round(sum(r.duration_in_seconds) / 3600.0, 1) AS width_hours
FROM (VALUES (0), (1), (2)) w(days)
CROSS JOIN LATERAL mock.get_impose_plan(p_until => '2026-09-07 06:00+02', p_line_type => 'sheet', p_look_back_days => w.days, p_look_ahead_days => w.days) r
