-- ============================================================
-- No noop rows on the material boards (7 sep). action.get_plan_lanes (material
-- mode) returned one row per tenant for the night window 00:00-06:00 from
-- action.non_working_times (type noop): removed time the client laid the
-- timeline around. The time scale (production.get_timeline_view_segments,
-- view nest-time-scale) now carries the axis itself — 06:00 to 24:00 plus the
-- previous evening from 21:45, the rush markers as segments — so the rows are
-- gone: get_plan_lanes gives material rows only, get_impose_plan has no
-- material-less case left (every row is a plan row with a lane item).
-- Data_groups 75 and 76 drop the hidden_when on material_id from their menus
-- and labels (sql/update_data_group_partial.sql, 75 + 76). Return types are
-- unchanged; the default of p_only_starting_today is true now (a caller that
-- leaves it out gets the materials whose interval says the day is a
-- production day). The breaks (type break) stay where they are: not read here.
-- ============================================================
...
--     lane_item.source_ref (<material_impose_plan_id>:<date>). One row per
--     material: the batch items of a lane are not rows here. No noop windows
--     any more: the non-working time is the time scale's
--     (get_timeline_view_segments), not a row. Feeds print_schedule and impose_plan
...
-- check: no row without a material or a lane item on board 76, the labels count the materials only
SELECT count(*) AS rows, count(*) FILTER (WHERE material_id IS NULL OR lane_item_id IS NULL) AS rows_without_material, count(*) FILTER (WHERE type <> 'plan') AS rows_without_kind
FROM mock.get_impose_plan(p_until => '2026-09-07 06:00+02', p_line_type => 'sheet');
SELECT count(*) AS labels, count(*) FILTER (WHERE material_id IS NULL) AS labels_without_material
FROM action.get_plan_lanes(p_until => '2026-09-07 06:00+02', p_line_type => 'sheet', p_only_starting_today => true);
