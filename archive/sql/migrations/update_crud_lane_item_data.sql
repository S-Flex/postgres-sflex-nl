-- The lane item mutation of the nest boards (76, 78) and their lane labels:
-- action.crud_lane_item reads one element per mutation with crud (create,
-- update, delete), track_by (the order of the mutations, returned on the
-- result) and a data object with the properties (lane_item_id,
-- start_offset_in_seconds, sort_order, is_pinned, and for a create lane_id,
-- plan_id, imposition_group_id). Until now the properties sat flat on the
-- element. The function becomes the stored_proc of the two data tables the
-- boards read, so a drop with commit "mutation" reaches it.
BEGIN;
...
SET stored_proc = 'action.crud_lane_item'
WHERE data_table IN ('get_impose_plan', 'get_plan_lanes_imposition_group')
  AND coalesce(stored_proc, '') <> 'action.crud_lane_item';
