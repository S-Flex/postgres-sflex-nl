-- ============================================================
-- catalog.imposition_group gets a parent_imposition_group_id (7 sep),
-- temporary: some materials are planned as one — the dibond dilite groups
-- under dibond digital, single sided under single sided, double sided under
-- double sided. The child keeps its own group (nests, settings, pattern);
-- the parent is where the readers that combine materials look. Nullable,
-- self-referencing, no cascade: removing a parent leaves the child on its own.
-- mapping.get_production_orderline_detail (the read behind the orderline
-- boards and the aggregate) widens a material filter with the groups planned
-- under the asked materials: a child whose parent is asked counts as that
-- material (section 3). The mapping (section 2) follows the groups in the
-- database — filled in once the tunnel is back; sections 1 and 3 can run.
-- ============================================================
...
ALTER TABLE catalog.imposition_group
    ADD COLUMN IF NOT EXISTS parent_imposition_group_id integer
        REFERENCES catalog.imposition_group (imposition_group_id);

COMMENT ON COLUMN catalog.imposition_group.parent_imposition_group_id IS
    'Temporary: the group this group is planned with (dibond dilite under dibond digital, single sided under single sided, double sided under double sided). Null = its own group. Goes when the xbom groups carry the combination themselves.';

CREATE INDEX IF NOT EXISTS idx_catalog_imposition_group_parent
    ON catalog.imposition_group (parent_imposition_group_id)
    WHERE parent_imposition_group_id IS NOT NULL;

-- 2. the mapping. The lanes and the pattern live on the dilite groups (300
--    "Dibond Digital/Dilite 3mm", 369 "... DZ"); the digital groups (28 single
--    sided, 29 double sided) have orders but no lane. The orderline read widens
--    from the asked material to its children, and a lane asks its own material,
--    so digital goes under dilite: single under single, double under double.
UPDATE catalog.imposition_group g
SET parent_imposition_group_id = v.parent_imposition_group_id
FROM (VALUES (28, 300),   -- sheet.dibond.digital.3mm.single.sided -> sheet.dibond.dilite.3mm
             (29, 369))   -- sheet.dibond.digital.3mm.double.sided -> sheet.dibond.dilite.3mm.double.sided
     AS v(imposition_group_id, parent_imposition_group_id)
WHERE g.imposition_group_id = v.imposition_group_id;

-- check: the pairs, with what each side holds
SELECT c.imposition_group_id AS child, c.item_code_paths::text AS child_paths,
       p.imposition_group_id AS parent, p.item_code_paths::text AS parent_paths,
       (SELECT count(*) FROM mapping.component_specs cs WHERE cs.material_id = c.imposition_group_id AND cs.is_open) AS child_open_orderlines
FROM catalog.imposition_group c
JOIN catalog.imposition_group p ON p.imposition_group_id = c.parent_imposition_group_id
ORDER BY 1;

-- 3. the orderline read follows the parent
