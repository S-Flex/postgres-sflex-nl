...
--      (p_step) resource, per shift in param_json.shifts (needs the shift codes of
--      sql/update_shift_code.sql),
...
VALUES ('oee-report', $json$[
  "availability = shift_duration - offline - has_break_times * break_times",
...
-- planned_operators 8 on 2026-09-14 for the Plaat line (day 4 + evening 3 + night 1), 45 on the aggregated summary row of Dokkum (no weekend planning), operator_cost_per_sqm > 0 where output > 0;
-- one summary row per tenant and day (set summary, resource_uid tenant-<id>) with the sums of its resources;
-- shifts 2 or 3 per row (the shift codes of the day), the rows per tenant by resource name;
...
SELECT r.business_date, r.report_key, r.set, r.production_line_id, r.resource_uids, r.tenant_name, r.resource_path,
       ...
       r.param_json ->> 'offline' AS offline,
       round((r.param_json ->> 'actual_net_output_sqm')::numeric) AS actual_net_output_sqm,
       round((r.param_json ->> 'actual_gross_output_sqm')::numeric) AS actual_gross_output_sqm,
       r.param_json ->> 'not_planned' AS not_planned,
       jsonb_array_length(r.param_json -> 'shifts') AS shifts,
...
ORDER BY r.tenant_id, (r.set = 'summary'), r.resource_name, r.sort_order, r.business_date
LIMIT 40;
