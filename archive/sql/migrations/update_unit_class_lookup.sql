-- The unit classes get a lookup (mapping.lookup / lookup_unit_class, source
-- json/lookup/mapping/lookup_unit_class.json): code, order, i18n with the title
-- and the abbreviation the boards show (SP single piece, MP multi piece, none
-- for all together). action.get_lane_item_work puts the node on every sub-list
-- entry as unit_class_json, so the tooltip of 76/78 reads the abbreviation in
-- the language of the user. relation.get_resources gets p_step for the printer
-- select of the inflow queue (79).
--
-- Runs after sql/update_impose_plan_cutoff_window.sql (same body of the work
-- reader, plus the lookup node).
BEGIN;

INSERT INTO mapping.lookup (lookup, lookup_json)
VALUES ('lookup_unit_class', $json$[
  {
    "code": "units-all",
    "order": 10,
    "i18n": {
...
-- ============ sql/relation/get_resources.sql ============
