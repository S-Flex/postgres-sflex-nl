-- ============================================================
-- log.get_resource_state_shift_totals counts p_days in shown days, not in
-- calendar days (7 sep). The window was v_until - p_days + 1 .. v_until with
-- weekends dropped afterwards, so a client asking one day with until at
-- saturday 00:00 (the end of friday) got an empty window and no rows. Now the
-- window is the last p_days dates on or before the day of p_until that count.
-- Weekends count by default now (there is production on them); with them
-- excluded, one day asked on a saturday is the friday. Mandatory days off
-- still only when asked. p_until is read as an exclusive instant: the client
-- sends the end of a day as the next midnight, which used to land on the next
-- day (the OEE chart of friday 4 sep came up empty). Only the machines of the
-- steps print and cut count (v_oee_steps in the function), whatever is asked.
-- New column tenant_name (the tenant of the machine, for the chart title of
-- data_group 62): the return type changes, recycle the hub afterwards.
-- ============================================================
...
begin
  ...
  -- only the machines of the OEE steps, asked for or not
  select array_agg(res.resource_uid)
  into p_resource_uids
  from relation.resource res
...
-- check: the client's call, until = the end of friday 4 sep; expected 3 rows on
-- 4 sep (available, planned, producing), OEE 42%
SELECT count(*) AS rows, min(shift_date) AS first_day, max(shift_date) AS last_day
FROM log.get_resource_state_shift_totals(p_resource_uids => array['PRdY9CJuOfyB'], p_until => '2026-09-05T00:00:00+02:00', p_line_type => 'sheet', p_states => array['producing'], p_days => 1, p_include_shifts => false, p_group_by => 'resource');
