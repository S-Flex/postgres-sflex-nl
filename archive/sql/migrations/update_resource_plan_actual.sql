-- ============================================================
-- Stap 7, vervolg (docs/plan-lane-model.md): de actual-rijen van
-- action.get_resource_plan worden items. Een run is de reeks geproduceerde
-- items van één batch achter elkaar (log.get_resource_produced; een nest zonder
-- batch is een eigen run; een batch die door een andere batch of door een gat
-- langer dan lookup_lane_item_type.actual.gap_split_in_seconds onderbroken
-- wordt, wordt twee runs). De stukken tussen de runs zijn ook items. De
-- statusblokken (log.get_resource_state) zijn het subniveau van elk actual-item
-- (states_json, geknipt op het item); de status die het item het meest vult
-- geeft state_json en de class. De lookup krijgt de drempel (900 s).
...
-- tooltip; its state (state_json, class_names) is the least advanced status of
-- its nests, its sub level the part statuses (part_status_json). type_json is the lookup node of the row's type; its class_names
...
begin
    ...
    rows as (
        ...
        select p.tenant_id, p.tenant_name, p.production_company_id,
               ...
               p.state_json, p.group_state_json, null::jsonb as states_json, p.class_names, p.param_json
        from plan_row p
        ...
        select p.tenant_id, p.tenant_name, p.production_company_id,
               ...
               p.state_json, p.group_state_json, null::jsonb, p.class_names, p.param_json
        from plan_row p
...
-- check 1: the kinds; expected on 2026-09-04 10:00 sheet: plan 150, progress 52,
-- actual 154 (74 runs and 80 stretches, was 838 raw rows)
SELECT type, count(*) AS rows,
       count(*) FILTER (WHERE (param_json ->> 'is_run')::boolean) AS runs,
       count(*) FILTER (WHERE states_json IS NOT NULL) AS with_states
FROM action.get_resource_plan('2026-09-04 10:00+02', 'sheet')
GROUP BY type ORDER BY type;

-- check 2: the actual items of a lane cover its day without overlap; expected:
-- every lane covered from 0 s, overlaps 0 (a run ends where the next begins)
SELECT resource_uid, sum(duration_in_seconds) AS covered, min(start_offset_in_seconds) AS first,
       max(start_offset_in_seconds + duration_in_seconds) AS last,
       count(*) FILTER (WHERE start_offset_in_seconds < prev_end) AS overlaps
FROM (SELECT *, lag(start_offset_in_seconds + duration_in_seconds) OVER (PARTITION BY resource_uid ORDER BY start_offset_in_seconds) AS prev_end
      FROM action.get_resource_plan('2026-09-04 10:00+02', 'sheet', NULL, NULL, array['actual'])) x
GROUP BY resource_uid ORDER BY resource_uid;
