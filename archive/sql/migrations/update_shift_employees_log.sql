-- Shift employees per shift (docs/plan-shift-employees.md, 15 Sep 2026):
--   1. lookup_shift moves from legacy.lookup to log.lookup ([{code, i18n}], with uk);
--      log.lookup lookup_absence is new (sick, leave; the titles of the absent
--      sections). json/lookup/log/<lookup>.json are the source.
--   2. log.get_resource_shift_employees(p_production_line_id, p_business_date)
--      replaces legacy.get_resource_shift_employees: the shift from the code in
--      the planning, a section per group or absence, one row per employee; the
--      data_table follows.
--   3. mapping.get_status_bar_teams on the new planning: the current shift only.
--   4. log.get_oee_report: the shift titles from log.lookup (broken since the
--      lookup lost its block today).
--   5. data_group resource_shift_employees (42): a grid column per shift present,
--      a container per group, then sick, then leave, the employee cards under it.
-- Rollback: sql/update_shift_employees_log_down.sql.
BEGIN;

-- ============ 1. the lookups ============
INSERT INTO log.lookup (lookup, lookup_json)
VALUES ('lookup_shift', $json$
[
  {
    "code": "day",
    "i18n": {
...
DELETE FROM legacy.lookup WHERE lookup = 'lookup_shift';

-- ============ 2. sql/log/get_resource_shift_employees.sql ============
...
SET query           = 'log.get_resource_shift_employees',
    description     = 'the employees planned on a production line on a business date, per shift and group, the absent in their own section',
    data_table_json = '{"primary_keys": ["shift_planning_id", "shift", "group_name", "employee_id"]}'::jsonb
WHERE data_table = 'get_resource_shift_employees';

-- ============ 3. sql/mapping/get_status_bar_teams.sql ============
...
-- ============ 5. data_group resource_shift_employees ============
...
-- expected: Plaat yesterday: per shift the groups and the absent with their counts; the status bar teams of the line; the OEE shift titles
SELECT shift, section, count(*) AS employees, min(marking) AS marking
FROM log.get_resource_shift_employees((SELECT min(line_id) FROM relation.production_line WHERE line_type = 'sheet' AND tenant_id = 1), current_date - 1)
GROUP BY shift_order, shift, section_order, section
ORDER BY shift_order, section_order, section;

SELECT mapping.get_status_bar_teams((SELECT min(line_id) FROM relation.production_line WHERE line_type = 'sheet' AND tenant_id = 1), current_date) AS teams;

SELECT DISTINCT e -> 'i18n' -> 'nl' ->> 'title' AS shift_title
FROM log.get_oee_report(p_date := current_date - 1, p_line_type := 'sheet') r
CROSS JOIN LATERAL jsonb_array_elements(r.param_json -> 'shifts') e;
