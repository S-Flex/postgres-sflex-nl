-- legacy.crud_nest crashed on every nest sync since the one-batch-per-lane-item
-- change (commit af361e9): "column reference sort_order is ambiguous" in the
-- new_set CTE, because the function returns a sort_order column and the CTE
-- read the column of kept without an alias. The sync of the single products
-- failed right after it on the nest the crashed call never inserted.
--
-- Fix: #variable_conflict use_column (the rule for every RETURNS TABLE
-- function) and the alias on kept. Nothing else changes.
BEGIN;
