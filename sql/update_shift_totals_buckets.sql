-- The buckets of lookup_resource_state changed on 9 sep 2026 (log.lookup,
-- json/lookup/log/lookup_resource_state.json): breakdown and offline states
-- count as 'unavailable', idle/starved/blocked as 'available' (the machine
-- could run and does not: the loss). log.get_resource_state_shift_totals
-- still summed 'breakdown', 'offline' and 'not-producing', so the
-- unavailable band lost missingdata and the losses were no longer drawn.
-- The function follows the lookup: one unavailable bucket (counts_as
-- unavailable, and offline while that state still says so), the loss bucket
-- is available. oee_json: unavailable_in_seconds is a parameter now,
-- breakdown_in_seconds / offline_in_seconds and their percentages are gone,
-- unavailable_percentage replaces them.
BEGIN;
...
-- check: one day of one line, the buckets summed; expected unavailable > 0 where missingdata/breakdown occur
SELECT state, round(sum(duration_seconds) / 3600, 1) AS hours, max(counts_as) AS counts_as
FROM log.get_resource_state_shift_totals(p_days => 1, p_line_type => 'sheet', p_group_by => 'line')
