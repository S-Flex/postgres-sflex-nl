-- The nest sync and the single-product sync of the hub (from the MySQL
-- source) run independently, and the single products run ahead: a single
-- product of a nest that is not here yet failed on the foreign key to
-- legacy.nest ("Key (nest_id)=(2437804) is not present in table nest"), and
-- with it the whole batch. A backfill of the nests is slower still.
--
-- 1. legacy.single_product loses the foreign key to legacy.nest: a single
--    product never checks that its nest exists. (The key cascaded deletes;
--    sql/delete_old_nests.sql already keeps nests with single products.)
-- 2. legacy.update_nest_commercial_waste: the waste computation, once, for
--    both writers.
-- 3. legacy.crud_single_product: no dependency on the nest. A nest that is
--    here gets its waste and its sheet manifest refreshed.
-- 4. legacy.crud_nest: create, merge and update are one upsert, so an
--    update of a nest that is not here yet inserts it (the fields an update
--    may not carry keep their value; domain_id falls back to 1). The single
--    products that came first are completed (nest_amount, waste).
BEGIN;

ALTER TABLE legacy.single_product DROP CONSTRAINT IF EXISTS single_product_nest_nest_id_fk;

-- ============ sql/legacy/update_nest_commercial_waste.sql ============
...
-- ============ sql/legacy/crud_single_product.sql ============
drop function if exists legacy.crud_single_product(jsonb, boolean);
...
-- check: no single product without nest_amount for nests that are here; expected 0
SELECT count(*) AS single_products_missing_nest_amount
FROM legacy.single_product sp
JOIN legacy.nest n ON n.nest_id = sp.nest_id
WHERE sp.single_product_json ->> 'nest_amount' IS NULL AND n.amount IS NOT NULL;

-- check: single products whose nest is not here yet (the backfill fills these)
SELECT count(*) AS single_products_without_nest, min(sp.created_at) AS oldest
FROM legacy.single_product sp
WHERE NOT EXISTS (SELECT 1 FROM legacy.nest n WHERE n.nest_id = sp.nest_id);
