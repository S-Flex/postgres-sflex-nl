-- Nest waste chart (docs/plan-nest-waste-ranges.md): the tooltip of
-- nest_waste_ranges_chart in the group form. The bars sum their segments over
-- every row behind an x position (one row per material, day and range), the
-- tooltip did not: its plain field_config form showed one of those rows. Now
-- header.group and sections[].group with an aggregate_fn per field, trigger
-- group, like production_board_graph. The rest of the data_group is the live
-- version of 14 Sep 2026 (the waste-* colour variables), unchanged.
-- Rollback: sql/update_nest_waste_chart_tooltip_down.sql.
BEGIN;
