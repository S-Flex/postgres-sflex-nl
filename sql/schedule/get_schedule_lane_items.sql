-- The items of the schedule boards (docs/schedule-base.md §7): one row
-- per item on the lanes whose day is in view. The kind of an item (plan,
-- progress, actual) is its own lane_item_type; step 2 writes plan items only,
-- the progress and actual items follow when the boards move over (step 3),
-- p_types filters on them.
--
-- Per row:
...
BEGIN
    ...
    item AS (
        SELECT ln.plan_id, ln.lane_id, ln.lane_date, ln.step, ln.resource_path,
               li.lane_item_type, li.lane_item_id, li.sort_order,
        ...
        WHERE (p_types IS NULL OR li.lane_item_type = ANY (p_types))
    ),
