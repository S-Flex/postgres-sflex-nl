-- The lanes (labels) of the schedule boards (docs/schedule-base.md §7):
-- one row per lane whose day is in view. A lane is one step on one machine on
-- one day; the kind of the work (plan, progress, actual) sits on the item, so
-- a lane has no type of its own. The material boards group its items, not its
-- lanes. The formula and constants of the machine
...
-- p_line_type null = every line type,
-- p_tenant_ids null = every tenant (the tenant of a lane is the first label of
-- its path, site.tenant.abb).
-- the lane_type went to schedule.lane_item.lane_item_type, so the signature
-- loses p_types and the return loses lane_type and type_json
drop function if exists schedule.get_schedule_lane(date, date, text, integer[], text[], text[]);
drop function if exists schedule.get_schedule_lane(date, date, text, integer[], text[]);

create function schedule.get_schedule_lane(p_from date DEFAULT current_date, p_until date DEFAULT current_date, p_line_type text DEFAULT NULL::text, p_tenant_ids integer[] DEFAULT NULL::integer[], p_steps text[] DEFAULT NULL::text[])
...
alter function schedule.get_schedule_lane(date, date, text, integer[], text[]) owner to xfw3;
