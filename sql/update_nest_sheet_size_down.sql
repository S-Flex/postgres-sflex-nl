-- Rollback of sql/update_nest_sheet_size.sql: legacy.crud_nest as before (the
-- payload's width and height as they come). The sizes set in the table stay:
-- the nested area is not kept anywhere once overwritten.
BEGIN;
