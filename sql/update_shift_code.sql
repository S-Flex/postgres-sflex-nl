-- Shift codes (docs/plan-oee-report.md §10): every shift gets a code, so the
-- OEE report can show a column per shift.
--   1. action.dates.shift_json (the source of the shifts): a `code` per shift,
--      day / evening / night, the codes of legacy lookup_shift in their order.
--      The rank of a shift is its start within the shifts of the same tenants
--      on that date; a start before 06:00 is the night that runs into the
--      morning and ranks last. A date with more shifts per tenants than the
--      lookup has codes keeps those shifts without a code: the preview shows them.
--   2. log.state_shift_agg.shift_code: the code of the window a row was bucketed
--      into. log.upsert_state_shift_agg already clips every state, plan item
--      and production job to the shift windows (a plan item from 14:00 to 15:30
--      with a shift change at 15:00 already gives two pieces); it now also
--      writes the code of the window.
--   3. Output in two areas: actual_output_sqm becomes actual_gross_output_sqm, and
--      actual_net_output_sqm is new: the production area less the waste of the
--      nest (legacy.nest.nest_json waste_percentage), the area of the customer
--      orders. Both on the producing row, written by the upsert.
--   4. Backfill: the aggregate rebuilt per date from 7 Sep 2026 (delete-then-insert
--      per date, so this is safe to rerun).
-- Rollback: sql/update_shift_code_down.sql.

-- ============ preview: the codes the shifts of the last 14 days get ============
-- expected: every shift a code; a null code marks a date to look at
WITH el AS (
    SELECT d.date, sh.ordinality AS idx, sh.value AS shift,
           dense_rank() OVER (PARTITION BY d.date, coalesce(sh.value -> 'tenants', '[]'::jsonb)
                              ORDER BY CASE WHEN (sh.value ->> 'start_offset')::integer < 21600
                                            THEN (sh.value ->> 'start_offset')::integer + 86400
                                            ELSE (sh.value ->> 'start_offset')::integer END) AS rnk
    FROM action.dates d
    CROSS JOIN LATERAL jsonb_array_elements(d.shift_json) WITH ORDINALITY AS sh(value, ordinality)
    WHERE d.shift_json IS NOT NULL
      AND d.date BETWEEN current_date - 14 AND current_date
),
codes AS (
    SELECT c.ordinality AS rnk, c.value ->> 'code' AS code
    FROM legacy.lookup lk
    CROSS JOIN LATERAL jsonb_array_elements(lk.lookup_json) WITH ORDINALITY AS c(value, ordinality)
    WHERE lk.lookup = 'lookup_shift'
)
SELECT el.date, el.idx, el.shift -> 'tenants' AS tenants, el.shift ->> 'start_offset' AS start_offset,
       el.shift ->> 'shift_duration' AS shift_duration, el.shift ->> 'code' AS current_code, c.code AS new_code
FROM el
LEFT JOIN codes c ON c.rnk = el.rnk
ORDER BY el.date, el.idx;

BEGIN;

-- ============ 1. action.dates.shift_json: the code per shift ============
WITH el AS (
    SELECT d.date, sh.ordinality AS idx, sh.value AS shift,
           dense_rank() OVER (PARTITION BY d.date, coalesce(sh.value -> 'tenants', '[]'::jsonb)
                              ORDER BY CASE WHEN (sh.value ->> 'start_offset')::integer < 21600
...
coded AS (
    SELECT el.date,
           jsonb_agg(CASE WHEN c.code IS NULL THEN el.shift ELSE el.shift || jsonb_build_object('code', c.code) END
                     ORDER BY el.idx) AS shift_json
    FROM el
    LEFT JOIN codes c ON c.rnk = el.rnk
    GROUP BY el.date
)
UPDATE action.dates d
SET shift_json = coded.shift_json
FROM coded
WHERE coded.date = d.date
  AND d.shift_json IS DISTINCT FROM coded.shift_json;

-- ============ 2. log.state_shift_agg.shift_code ============
ALTER TABLE log.state_shift_agg ADD COLUMN IF NOT EXISTS shift_code text;
COMMENT ON COLUMN log.state_shift_agg.shift_code IS 'the code of the shift window (action.dates.shift_json code: day, evening, night)';

-- ============ 3. the output in two areas ============
DO $ddl$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns
               WHERE table_schema = 'log' AND table_name = 'state_shift_agg' AND column_name = 'actual_output_sqm') THEN
        ALTER TABLE log.state_shift_agg RENAME COLUMN actual_output_sqm TO actual_gross_output_sqm;
    END IF;
END
$ddl$;
ALTER TABLE log.state_shift_agg ADD COLUMN IF NOT EXISTS actual_net_output_sqm numeric;
COMMENT ON COLUMN log.state_shift_agg.actual_gross_output_sqm IS 'nest sheet area produced in the shift (jobs starting in it, amount x nest width x height), on the producing row';
COMMENT ON COLUMN log.state_shift_agg.actual_net_output_sqm IS 'the produced area less the waste of the nest (nest_json waste_percentage): the area of the customer orders, on the producing row';
...
-- ============ 4. backfill: the aggregate per date since 7 Sep 2026 ============
DO $do$
DECLARE
    v_day  date;
    v_rows integer;
...
-- ============ check ============
-- expected: every row since 7 Sep has a code; the customer-order area is below the production area
SELECT shift_date, shift_code, count(*) AS rows, count(*) FILTER (WHERE shift_code IS NULL) AS without_code,
       round(sum(actual_gross_output_sqm)) AS actual_gross_output_sqm, round(sum(actual_net_output_sqm)) AS actual_net_output_sqm
FROM log.state_shift_agg
WHERE shift_date >= date '2026-09-07'
GROUP BY 1, 2
ORDER BY 1, 2;
