-- Rollback of sql/update_planning_template.sql (14 Sep 2026): the eight
-- functions as they were live before the script, the old key on
-- legacy.imposition_group, the xbom config from the backup, the leads back on
-- catalog.item_group_resource. The tenant 2 rows and the rules go.

begin;

-- the column name first: the sql-language bodies are checked on creation
alter table legacy.imposition_group rename column rules_json to imposition_group_json;

-- the rules and the tenant 2 rows go
delete from legacy.imposition_group where tenant_id <> 1;
update legacy.imposition_group
set imposition_group_json = imposition_group_json - 'delivery_hours' - 'min_delivery_hours' - 'unit_threshold' - 'schedules'
where imposition_group_json ?| array['delivery_hours', 'min_delivery_hours', 'unit_threshold', 'schedules'];
alter table legacy.imposition_group alter column imposition_group_json drop default;

-- the old key
alter table legacy.imposition_group
    drop constraint imposition_group_parent_imposition_group_id_fkey,
    drop constraint imposition_group_tenant_item_code_paths_key,
    drop constraint imposition_group_pkey;
alter table legacy.imposition_group
    add constraint imposition_group_pkey primary key (imposition_group_id),
    add constraint imposition_group_item_code_paths_key unique (item_code_paths),
    add constraint imposition_group_parent_imposition_group_id_fkey
        foreign key (parent_imposition_group_id) references legacy.imposition_group (imposition_group_id);
alter table action.imposition_group_lane_item
    add constraint imposition_group_lane_item_imposition_group_id_fkey
        foreign key (imposition_group_id) references legacy.imposition_group (imposition_group_id);
alter table legacy.imposition_group drop column tenant_id;

-- the functions as they were live before the script; the old
-- get_imposition_group names the restored unique constraint, so the key goes first
drop function if exists legacy.get_imposition_group(text[], integer);
CREATE OR REPLACE FUNCTION legacy.get_imposition_group(p_option_codes text[])
 RETURNS integer
 LANGUAGE sql
AS $function$
...
alter function legacy.get_imposition_group(text[]) owner to xfw3;

CREATE OR REPLACE FUNCTION legacy.create_nest_manifest(p_nest_ids bigint[])
 RETURNS void
 LANGUAGE sql
AS $function$
WITH step_order AS (
...
CREATE OR REPLACE FUNCTION mapping.get_materials(p_production_line_ids integer[] DEFAULT NULL::integer[], p_line_type text DEFAULT NULL::text)
 RETURNS TABLE(material_id integer, material_name text, production_line_ids integer[])
 LANGUAGE sql
 STABLE
AS $function$
...
CREATE OR REPLACE FUNCTION mapping.get_production_orderline_manifest(p_material_id integer, p_date date DEFAULT CURRENT_DATE, p_look_ahead_days integer DEFAULT '-1'::integer, p_threshold integer DEFAULT 1, p_domain_id integer DEFAULT 1, p_tenant_ids integer[] DEFAULT NULL::integer[])
...
CREATE OR REPLACE FUNCTION mapping.update_component_specs_manifest(p_production_orderline_ids integer[])
 RETURNS integer
 LANGUAGE sql
AS $function$
...
CREATE OR REPLACE FUNCTION action.get_plan_lanes_imposition_group(p_until timestamp with time zone DEFAULT now(), p_step text DEFAULT 'impose'::text, p_line_type text DEFAULT NULL::text, p_tenant_ids integer[] DEFAULT NULL::integer[], p_only_starting_today boolean DEFAULT true, p_view_code text DEFAULT 'nest-time-scale'::text)
 RETURNS TABLE(imposition_group_id integer, material_id integer, material_name text, production_line_id integer, tenant_id integer, tenant_name text, resource_path ltree, resource_uid text, resource_name text, delivery_hours integer, min_delivery_hours integer, day_offset integer, plan_date date, sort_order numeric, param_json jsonb, formula jsonb, data jsonb, fixed_group text, is_pinned boolean, start_offset_in_seconds integer, next_start_offset_in_seconds integer, lane_item_id bigint, lane_id bigint, instance integer, status text, nest_moment_code text, nest_time time without time zone, print_time time without time zone)
 LANGUAGE plpgsql
 STABLE
AS $function$
#variable_conflict use_column
...
-- the xbom config back
update catalog.xbom x
set config_json = b.config_json
from catalog.xbom_config_backup_20260914 b
where b.xbom_id = x.xbom_id;
drop table catalog.xbom_config_backup_20260914;

-- the leads back
alter table catalog.item_group_resource add column lead_in integer, add column lead_out integer;
