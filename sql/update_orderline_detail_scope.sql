-- ============================================================
-- Stap 1 van docs/plan-lane-model.md: de nest-planning (bord 76) sneller.
-- mapping.get_production_orderline_detail resolves the orderlines of a nest
-- or batch scope once (scope_orderline) and hands them to the base query as
-- an array, so the planner stops estimating 28.446 rows for 566 and uses the
-- indexes on production_orderline_id instead of scanning single_product,
-- production_orderline_progress and spec_unit_manifest in full per set call.
-- Measured as a plain query: one set of 232 nests 1.285 ms -> 255 ms, board
-- window unchanged (nested loops, ~0,8 s). Output identical in both scopes.
-- The mirror below drops its own old signatures.
-- ============================================================
...
-- check 1: the nest board; expected: < 0,5 s (was 1,9 s), 48 rows on a workday
EXPLAIN (ANALYZE, SUMMARY)
SELECT * FROM mock.get_impose_plan('2026-09-04 10:00+02', 'print', 'sheet', NULL, 0, 0, 1);

-- check 2: the production board window did not slow down; expected: ~1 s
EXPLAIN (ANALYZE, SUMMARY)
SELECT * FROM mapping.get_production_board_aggregate(p_from => '2026-09-04 10:00+02');

-- check 3: one set call in nest scope; expected: < 300 ms
EXPLAIN (ANALYZE, SUMMARY)
SELECT * FROM mapping.get_production_orderline_aggregate(
         p_from => '2026-09-04 10:00+02', p_date_type => 'nest',
         p_nest_ids => (SELECT array_agg(DISTINCT imposition_id) FROM action.imposition_lane_item
                        WHERE lane_item_id = (SELECT max(lane_item_id) FROM action.imposition_lane_item)),
         p_tenant_ids => array[1,2], p_status_sequences => NULL, p_is_open => NULL, p_domain_id => 1);
