-- lookup_resource_state: idle, starved, starved.operator, blocked and
-- blocked.operator count as 'not-producing'. Until now they had no counts_as
-- and the OEE read took "no counts_as" as the loss bucket; it now names the
-- bucket, and the states filter (log.get_resource_states: every node with a
-- counts_as) offers them again.
--
-- Source of truth: json/lookup/log/lookup_resource_state.json, inline below.
-- Run before or after sql/update_shift_json_format.sql: both carry the same
-- body of log.get_resource_state_shift_totals.
BEGIN;
